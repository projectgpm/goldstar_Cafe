...
namespace QLCafe.DAO
{
    public class DAO_PhaChe
    {
        ...
        public List<DTO_DanhSachChuaPhaChe> GetDanhSachMonAn()
        {
            ...
            string sTruyVan = string.Format(@"  SELECT [CF_HangHoa].MaHangHoa, [CF_HangHoa].TenHangHoa,[CF_DonViTinh].TenDonViTinh, [CF_ChiTietHoaDon_Temp].SoLuong,[CF_ChiTietHoaDon_Temp].ID,[CF_ChiTietHoaDon_Temp].TrangThai,[CF_Ban].TenBan FROM [CF_Ban],[CF_ChiTietHoaDon_Temp],[CF_HangHoa],[CF_DonViTinh] WHERE [CF_Ban].ID = [CF_ChiTietHoaDon_Temp].[IDBan] AND [CF_HangHoa].IDDonViTinh = [CF_DonViTinh].ID AND [CF_ChiTietHoaDon_Temp].IDHangHoa = [CF_HangHoa].ID AND [CF_ChiTietHoaDon_Temp].TrangThai = 0");
