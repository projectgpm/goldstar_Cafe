...
namespace QLCafe.DAO
{
    public class DAO_KetCa
    {
        ...
        public static bool ThemKetCa(int IDNhanVien, string GioVao, string GioRa, double TongTien, string IDChiNhanh)
        {
            string sTruyVan = string.Format(@"INSERT INTO CF_KetCa(ThoiGianBatDau,ThoiGianKetThuc,TongTienSauCa,IDNhanVien,IDChiNhanh) VALUES ('{0}','{1}','{2}','{3}','{4}')", DateTime.Parse(GioVao).ToString("yyyy-MM-dd hh:mm:ss tt"), DateTime.Parse(GioRa).ToString("yyyy-MM-dd hh:mm:ss tt"), TongTien, IDNhanVien, IDChiNhanh);
            return DataProvider.TruyVanKhongLayDuLieu(sTruyVan);
        }
    }
}
