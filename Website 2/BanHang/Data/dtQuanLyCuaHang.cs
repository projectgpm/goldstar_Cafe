...
namespace BanHang.Data
{
    public class dtQuanLyCuaHang
    {
        ...
        public static DataTable TongTienHienTai(string GioBD, string GioKT)
        {
            ...
            {
                con.Open();
                string cmdText = "SELECT SUM(TongTien) as TongTien,SUM(TongGiamGia) as TongGiamGia,SUM(TienPhuThu) as TienPhuThu FROM CF_HoaDon WHERE GioVao >= '" + GioBD + "' AND GioVao <= '" + GioKT + "' AND [TrangThai] = 1";
                using (SqlCommand command = new SqlCommand(cmdText, con))
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    DataTable tb = new DataTable();
