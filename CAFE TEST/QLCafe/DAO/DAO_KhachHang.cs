...
namespace QLCafe.DAO
{
    public class DAO_KhachHang
    {
        ...
        public List<DTO_KhachHang> listKhachHang()
        {
            ...
            string sTruyVan = string.Format(@"SELECT * FROM [GPM_KhachHang] WHERE DAXOA = 0");
        ...
        public static int KiemTraMaKhachHang(string MaKhachHang)
        {
            string sTruyVan = string.Format(@"SELECT * FROM [GPM_KHACHHANG] WHERE MaKhachHang =  N'{0}'", MaKhachHang);
            DataTable data = new DataTable();
            data = DataProvider.TruyVanLayDuLieu(sTruyVan);
            if (data.Rows.Count == 0)
                return 0;
